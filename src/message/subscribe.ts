 ...
 * draft-ietf-moq-transport-21 Section 9.6 (SUBSCRIBE) — 9.7 (SUBSCRIBE_OK) — 9.5 (REQUEST_UPDATE)
 */
...
/**
 * SUBSCRIBE メッセージ (Section 9.6 SUBSCRIBE)
 *
...
/**
 * SUBSCRIBE_OK メッセージ (Section 9.7 SUBSCRIBE_OK)
 *
 * draft-ietf-moq-transport-21:
...
/**
 * REQUEST_UPDATE メッセージ (Section 9.5 REQUEST_UPDATE)
 *
 * draft-ietf-moq-transport-21:
...
export function decodeSubscribePayload(data: Uint8Array, offset = 0): Subscribe {
  ...
  if (offset + totalConsumed + Number(nameLen) > data.length) {
    throw new ProtocolViolationError(
      `subscribe track name length exceeds remaining data: ${nameLen} > ${data.length - (offset + totalConsumed)}`,
    );
  }
