...
export function decodePublishPayload(data: Uint8Array, offset = 0): Publish {
  ...
  assertLengthWithinData("publish track name", nameLen, offset + totalConsumed, data.length);
...
export function decodePublishDonePayload(data: Uint8Array, offset = 0): PublishDone {
  ...
  assertLengthWithinData("reason phrase", reasonLen, offset + totalConsumed, data.length);
