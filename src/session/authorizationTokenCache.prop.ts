/**
 * AuthorizationTokenCache Property-Based Tests
  ...
  fc.assert(
    fc.property(
      ...
      (alias, tokenType, tokenValue) => {
        const cache = new AuthorizationTokenCache(0n);
  ...
  fc.assert(
    fc.property(
      ...
      (alias, tokenType, tokenValue) => {
        ...
        const cache = new AuthorizationTokenCache(size);
...
test("同じ alias の二重 REGISTER は DUPLICATE_AUTH_TOKEN_ALIAS", () => {
  const cache = new AuthorizationTokenCache(100n);
...
test("容量を一部超えると新規 REGISTER は false になる", () => {
  ...
  const cache = new AuthorizationTokenCache(30n);
