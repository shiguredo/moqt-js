...
- 同じ draft-21 を実装する moqt-rs も 0x17 を Session Termination としてのみ持ち、未登録 Alias の参照でセッションを閉じる。relay も 0x17 を Session Termination のコードとして扱う方針である。詳細は「他実装との解釈合わせ」を参照
...
relay は 0x17 を Session Termination のコードとして定義し、未登録 Alias を Session Termination とする方針である。トークンキャッシュの実装は未着手であり、この一致は実装済みの挙動ではなく解釈と方針の一致である。
