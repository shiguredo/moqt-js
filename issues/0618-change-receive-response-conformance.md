...
- 確立後の 2 通目の REQUEST_OK を黙殺する。§6.4.2.2 は複数受信時に protocol error で閉じる SHOULD を定め、namespace 系ループは閉じる実装になっている
...
- 2 通目 REQUEST_OK は「pending がなく、自 endpoint が REQUEST_UPDATE を送っていない」場合に限り PROTOCOL_VIOLATION とする (§9.5 が認める coalescing と区別する)
