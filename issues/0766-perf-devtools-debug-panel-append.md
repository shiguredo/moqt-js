...
- 1 件追加の所要時間が半分以下になる (実測: 同じ計測方法で 1000 件表示の中央値 18.4 ms → 7.2 ms)
...
- 行の描画は 1 件になったが、Preact が表示中のすべての子 (行の vnode) を走査する分は件数に比例して残る (実測: 100 件 0.4 ms / 300 件 1.4 ms / 1000 件 7.2 ms)。この分を無くすには、表示する行を画面に入る分だけにする (仮想化) か、表示件数を絞る (WebTransport のメッセージ一覧は 200 件) 必要がある。表示の内容を変える判断が要るため別 issue にする
