...
draft-ietf-moq-transport-21 §9.13 は「A potential subscriber sends TRACK_STATUS as the first and only message on a new bidi stream to obtain information about the current status of a given track.」「If successful, the publisher responds with a TRACK_STATUS_OK with the same parameters and Track Properties it would have set in a SUBSCRIBE_OK.」と定める。TRACK_STATUS の送信者は potential subscriber であり、TRACK_STATUS_OK は SUBSCRIBE_OK と同じ Track Properties を運ぶ。§12.1 は「There are multiple ways a publisher can transmit a Track that does not conform to MOQT constraints. Such a Track is considered malformed.」と定義し、「When a subscriber detects a Malformed Track, it MUST cancel any corresponding subscription or fetches for that Track from that publisher (see Section 6.4.2.3), and SHOULD deliver an error to the application.」と定める。未知 Mandatory Track Property (0x4000-0x7FFF) を TRACK_STATUS_OK で受信することは、§3.6 が「MUST NOT process or forward that track」とする Track の検出であり、§9.13 が SUBSCRIBE_OK と同一の Track Properties を運ぶと定めている以上、§12.1 の malformed Track の検出に当たる。しかし現状は pending の reject のみで、bidi ストリームの FIN (§9.13 / §6.4.2.2) も、同一 Track の購読 / FETCH の cross-cancel (§12.1 の MUST) も行われない。

§3.6 が未知 Mandatory Track Property の扱いとして列挙するのは PUBLISH / SUBSCRIBE_OK / FETCH_OK であり TRACK_STATUS_OK を含まないが、これは §12.1 の malformed Track 検出の適用を排除しない (cross-cancel の根拠は §12.1 の MUST と、§9.13 が TRACK_STATUS_OK に SUBSCRIBE_OK と同じ Track Properties を運ばせている点、および §3.6 の MUST NOT に置く)。
...
3. cross-cancel の比較キーを組み立てるため、`PendingTrackStatus` に Track の比較キー (`fullTrackNameKey` の戻り値) を追加し、`SessionImpl.trackStatus()` の pending 登録時に設定する (`fullTrackNameKey` は `src/fullTrackName.ts` が export しており `src/session.ts` から利用できる)。`resolve` / `reject` の既存の挙動は変えない。キーは `pendingTrackStatus` から削除する前に参照する (削除後も context が保持する pending オブジェクトから読めるが、順序を固定する)。なお `cancelMalformedTrackPeers` の走査対象は `subscribersByAlias` / `fetchers` / `pendingSubscribe` / `pendingFetch` であり、`pendingTrackStatus` は含まれない (TRACK_STATUS の pending を巻き込む挙動変化は起きない)。
...
5. テストを追加する: 同一 Full Track Name の購読 / FETCH を確立した状態で、未知 Mandatory Track Property を含む TRACK_STATUS_OK を実ストリームで注入し、(a) pending が `MalformedTrackError` で reject され、(b) 自方向が FIN され (`closeRequestStreamWriter` 相当の writer 終端)、(c) 同一 Track の購読 / FETCH が cancel され、(d) 別 Track の購読 / FETCH は active のまま、(e) `requestStreams` / `pendingTrackStatus` にエントリが残留しないことを検証する。既存の SUBSCRIBE_OK / FETCH_OK cross-cancel テストと同型の相手役を用意する。`createOkResponseReadTestContext` が内部マップを備えたセッションを提供するため、残留は直接観測できる。
...
- `fullTrackNameKey` (`src/fullTrackName.ts`。cross-cancel の比較キーを生成する)
