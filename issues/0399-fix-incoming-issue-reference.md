...
`src/session/incoming.ts` のモジュール doc に残る issue 番号参照（「issue 0302 設計方針参照」）を、shiguredo-issues 規約（issue 番号や issue への言及をソースコードに持ち込まないこと）に従い削除する。issue 番号ではなく、その理由（状態結合の強さ）を記述に置き換える。
...
- issue 番号参照を削除し、理由そのもの（状態結合が強いため）のみを残す。
- コード変更は伴わない。

## 完了条件

- `src/session/incoming.ts` のモジュール doc から issue 番号参照が削除され、理由のみの記述になっていること。
