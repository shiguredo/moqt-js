...
1. クリック直後 2 秒以内にコンポーネントがアンマウントされると、`copyButtonText.value = "Copy URL"` の代入がアンマウント後に走る。`useSignal` で作った signal はコンポーネントスコープのため動作上の致命的問題は無いが、不要な書き込みが発生する。
...
- issues/0170-fix-debug-panel-copy-settimeout-cleanup.md (`DebugPanel.tsx` の `setTimeout` クリーンアップ。本 issue とは対象ファイルが異なるが、考え方は同じ)
- issues/0173-refactor-extract-use-copy-feedback-hook-for-debug-panel.md (DebugPanel 用の汎用 `useCopyFeedback` hook 化。本 issue とはスコープが異なるため別 issue のまま並行で進める)
...
- `useRef<ReturnType<typeof setTimeout> | undefined>(undefined)` で timeout ID を保持する。
...
- `copy` 関数は安定参照である必要は無い (`onClick` に毎レンダ渡しても問題ない)。`useCallback` は不要。
- リセット遅延 (2000ms) と各文言 ("Copy URL" / "Copied!" / "Failed") は hook 内のローカル定数で定義する。引数化はしない (現時点で複数値を取る要件は無い)。
...
- 手動確認:
  ...
  - `webcodecs-devtools.html` にナビゲートして戻ってきた直後にコピーボタンを押し、警告コンソールに `Cannot set property of unmounted ...` 系の警告が出ないことを確認する (アンマウント後の signal 書き込みが起きないこと)。

将来 `@testing-library/preact` を導入できた段階で単体テストを追加する余地はあるが、本 issue のスコープ外とする。

## CHANGES.md 記載方針

- `### misc` サブセクションに 1 行で追加する。
- 種別は `[CHANGE]` (内部リファクタだが、`setTimeout` 解放を含むためユーザー観測動作にも影響しうる)。
- 文面例: `- [CHANGE] Copy URL ボタンのロジックを useCopyUrlButton hook に抽出し、アンマウント時の setTimeout を解放するという形で対応する`
