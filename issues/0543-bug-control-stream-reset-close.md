...
draft-ietf-moq-transport-21 §6.3 は「A control stream MUST NOT be closed at the underlying transport layer during the session's lifetime. Doing so results in the session being closed as a PROTOCOL_VIOLATION.」と定める。現状はピアの RESET_STREAM で制御ストリーム読み取りが reject しても、セッションを閉じずに半壊状態が残る。
...
- `src/session/errors.ts` の `isPeerStreamError` は `source === "stream"` を真とし、その JSDoc と `src/session/errors.test.ts` のコメントは「ピア起因の stream error はセッション終了 (PROTOCOL_VIOLATION) に昇格させない」としている。制御ストリームの RESET_STREAM は `isPeerStreamError` に該当するため、§6.3 の MUST を満たすにはこの規則の例外が必要になる。
...
3. 制御ストリームの RESET_STREAM は §6.3 により `isPeerStreamError` の「stream error は昇格させない」規則の例外であることを、`isPeerStreamError` の JSDoc と関連テストコメントに明記する。
...
- draft-ietf-moq-transport-21 §6.3 / §6.6 / §6.4.2.3
...
制御ストリームの読み取りループで peer 起因の RESET_STREAM を検出したとき、セッションを PROTOCOL_VIOLATION で閉じるようにした。

- `src/session.ts` に `closeControlStreamViolation` を追加し、FIN 経路と RESET 経路で `sessionState === "connected"` のガードを共通化する
- `startControlMessageLoop` の catch で `isPeerStreamError(err)` のとき `closeControlStreamViolation` を呼ぶ。`source: "session"` の read 失敗は昇格も通知もせず、それ以外の例外は従来どおり `notifyErrorIfActive` に委ねる
- `src/session/errors.ts` の `isPeerStreamError` JSDoc と `src/session/errors.test.ts` のコメントに、制御ストリームの RESET_STREAM が §6.3 の MUST による例外であることを追記する
- `src/session.test.ts` に RESET_STREAM で PROTOCOL_VIOLATION、既閉セッションで誤通知なし、アプリコールバック throw で閉じない、`source: "session"` で昇格・通知なしのテストを追加する
