...
`startControlMessageLoop()` で制御ストリームの `done` を検出した際に、`SessionError(PROTOCOL_VIOLATION)` をコールバックに通知し、`close()` でセッション内部ステートをクリーンアップした後、`transport.close()` で PROTOCOL_VIOLATION コードと共に WebTransport セッションを閉じるようにした。
