...
`Session` (低レベル) の `ConnectOptions` には issue 0098 で `authorizationToken` を追加済みだが、高レベル API である `createMediaPublisher` / `createMediaSubscriber` には未露出。認証付き Relay (例: relay) に高レベル API のまま接続する手段が無い。
