...
`@playwright/test` 1.59.1 と `e2e-test*` scripts は `package.json` に既に定義されているが、`playwright.config.ts` も spec ファイルも未作成で実体が無い。実 MOQT サーバ (relay 等) に対する WebTransport 接続を最小スコープで自動検証する E2E 基盤を導入する。
