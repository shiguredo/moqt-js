...
- `src/session/publish.ts` の `publishClosePublisherStreamInternal` は通常経路が `writer.close()`（FIN）であり、`writer.abort()`（reset 相当）は close の失敗・タイムアウト時のフォールバックに限られる。そのまま再利用すると FIN になり、§5.1.1 の reset MUST を満たさない。
