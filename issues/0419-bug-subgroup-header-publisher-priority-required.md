...
- これはワイヤ形式の違反 (Priority Present ありなのに Priority フィールドが無い) であり、ピアはデコードエラーになる。送信経路 (`src/session/publish.ts` は `params.priority ?? 128` で値を補う) では現実に発生しないが、ライブラリの公開関数として不正入力を検出できない。
