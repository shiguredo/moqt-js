...
`src/createMediaPublisher.ts` の `shouldSendKeyFrame` はフレーム番号の剰余でキーフレームを判定するため、`keyframeInterval: 0` では `frameCount % 0` が NaN になり常に false になる。周期キーフレームを一度も要求しなくなり、フレーム番号を 0 に戻すだけの `requestKeyframe()` も効かない。値域検証が無いため、負値は絶対値と同じ判定、非整数は値によって挙動が変わり (0.5 は毎フレーム true、1.25 は 5 フレームごとに true、1.1 は先頭以外 true にならない)、NaN は 0 と同じく一度も true にならず、±Infinity は先頭のみ true という意図しない挙動になる。
...
- 同型の剰余が devtools に 2 箇所ある (`devtools/src/hooks/usePublisher.ts` の `shouldRequestKeyFrame` と `devtools/src/webcodecs-devtools/signals.ts` のインライン剰余)。両画面の `keyframeInterval` は `<select>` なので画面操作からは無効値が入らないが、moqt-devtools の `initFromUrl` はクエリパラメータからの復元で `Number.parseInt` の結果を `!Number.isNaN` だけで検証するため `?keyframeInterval=0` で 0 が、`?keyframeInterval=-5` で負値がそのまま入る
...
- 解決後の値も検証する。`keyframeInterval` を指定しない場合の既定値は `Math.round(framerate * 2)` とし、整数に丸める。`shouldSendKeyFrame` は剰余で判定するため、非整数の間隔は値によっては周期キーフレームの要求が先頭の 1 回で止まる (`framerate: 29.97` の 59.94 がこれに当たる)。1 未満または非有限 (`framerate: 0` / 負値 / `NaN` / `±Infinity`) なら throw する
...
- `src/codec/types.ts` の `VideoPublishOptions.keyframeInterval` に、同ファイルの他のフィールドと同じ `//` 形式のコメントで値域 (1 以上の整数)、既定値 (`Math.round(framerate * 2)`、既定 framerate の 30 なら 60)、無効値は `createMediaPublisher()` が reject することを書く
...
- `framerate` 由来の既定値も 1 以上の整数に丸められ、`framerate` 未指定なら 60、`framerate: 29.97` なら 60 を返す
- `requestKeyframe()` が間隔 1 以上で次のフレームをキーフレームにする既存の挙動が変わらない
- `src/createMediaPublisher.test.ts` の `resolveKeyframeInterval` / `shouldSendKeyFrame` のテストが拡張され、拒否する 6 値・拒否する `framerate` の 0 / 負値 / 非有限・受理する境界値・`framerate: 29.97` の丸めが固定される
...
- 0676 (devtools の同型 2 箇所の値域検証) / 0680 (キーフレーム判定のあとに破棄されたフレームで要求が失われる件)
