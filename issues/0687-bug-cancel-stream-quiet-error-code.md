...
- コードを埋め込まない経路は 5 つある。`dataStreamHandleMalformedSubgroupTrack` (831-849 行目) は 845-848 行目で同じ malformed 打ち切りでありながら `code=` を含めない。fill の malformed (557-560 行目)、pending mode の abandon (922-925 行目 / 938-941 行目 / 954-957 行目)、`src/session/namespaceLoops.ts` の 304 行目も含めない
