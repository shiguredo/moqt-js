...
注: §5.4 は "Catalog field values MAY contain variables" の MAY であり、未知 field 置換の MUST / SHOULD は存在しない。§5.6 は非規範例 ("non-normative JSON examples")。本 issue は MAY の実装であり、仕様上の必須度は低い。
...
- `resolveCatalogVariables` (`src/msf.ts`) は既知キーの文字列を走査 (`substituteTrack` の `stringFields` 固定ホワイトリスト)
- **未知 field は `validateCatalog` / `buildValidatedCatalogTrack` で構造的に破棄されている**: root は `const catalog: Catalog = { version, tracks }` で既知 field のみ新規構築、track は `pick*` 群で既知 field のみ pick。§5 の "parser MUST ignore unknown" を「破棄」と解釈して実装している
...
- 注: §11.1.1 の `c4m` は base64 文字列だが、§5.4.1 の変数値文字集合 `[A-Za-z0-9_@-]` は base64 の `+` / `/` / `=` を含まない。実 c4m token は変数値として運べない可能性がある
