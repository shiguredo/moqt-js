...
> 前提の更新: END_OF_GROUP status の送信時に Subgroup ストリームを閉じる実装変更
> (`issues/closed/0611-bug-subgroup-stream-close-reset.md`) により、END_OF_GROUP 送信は
> 「ストリームを閉じない」前提ではなくなった。本 issue の着手時には、END_OF_GROUP 送信を
> FIN の契機として SUBGROUP_DELIVERY_TIMEOUT のタイマー開始点に含めるかを再評価すること
> (省略がある場合は RESET で閉じるため、FIN の契機は group 遷移と END_OF_GROUP の 2 つになる)。
