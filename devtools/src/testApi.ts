  ...
  const api: MoqtDevToolsApi = {
    ...
    getSubscribers: () =>
      Array.from(subscriberInstances.value.values()).map((sub) => ({
        id: sub.id,
        status: sub.status,
        framesDecoded: sub.framesDecoded,
        keyFramesDecoded: sub.keyFramesDecoded,
        objectsReceived: sub.objectsReceived,
        currentGroup: sub.currentGroup,
        currentSubGroup: sub.currentSubGroup,
        bytesReceived: sub.bytesReceived,
        objectsWithExtensions: sub.objectsWithExtensions,
        decoderState: sub.decoderState,
        joiningFetchStats: sub.joiningFetchStats,
        largestLocation: convertLargestLocation(sub.largestLocation),
      })),
