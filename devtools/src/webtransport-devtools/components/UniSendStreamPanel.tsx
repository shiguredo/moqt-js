...
function UniSendStreamItem({ streamId }: { streamId: number }) {
  ...
  const handleSend = () => {
    if (input.value.trim() && !stream.closed) {
      void store.sendUniMessage(streamId, input.value);
      input.value = "";
    }
  };
    ...
    >
      <div class="flex items-center justify-between mb-2">
        ...
        <div class="flex items-center gap-1">
          <ActionButton
            onClick={() => store.clearUniMessages(streamId)}
          ...
          {!stream.closed && (
            <ActionButton
              onClick={() => void store.closeUniStream(streamId)}
              title="Close stream"
              variant="danger"
            >
              <StopIcon />
          ...
          <ActionButton
            onClick={() => store.removeUniStream(streamId)}
 ...
 * 送信用単方向ストリームパネル
 */
export function UniSendStreamPanel() {
  const handleCreateStream = () => {
    void store.createUniStream();
  };
    ...
    <div class="bg-white rounded-xl shadow-sm p-5 mb-6">
      <div class="flex items-center justify-between mb-4">
        <h2 class="text-lg font-semibold text-slate-700 flex items-center gap-2">
          <OutgoingStreamIcon />
          Outgoing Streams
          {store.uniSendStreams.value.length > 0 && (
            <span class="text-xs bg-blue-100 text-blue-700 px-2 py-0.5 rounded-full">
              {store.uniSendStreams.value.length}
            </span>
          )}
        </h2>
        <button
          onClick={handleCreateStream}
          data-testid="uni-send-new-stream"
      ...
      <div class="mb-4 p-3 bg-slate-50 rounded-lg space-y-3">
        ...
        <div>
          <label for="uniSendOrder" class="block text-xs font-medium text-slate-600 mb-1">
            sendOrder
          </label>
          <input
            type="number"
            id="uniSendOrder"
            data-testid="uni-send-stream-send-order"
        ...
        <label class="flex items-center gap-2 text-xs text-slate-600">
          <input
            type="checkbox"
            data-testid="uni-send-stream-wait-until-available"
      ...
      {store.uniSendStreams.value.length === 0 ? (
        <p class="text-slate-500 text-sm">No outgoing streams</p>
      ) : (
        <div class="space-y-4">
          {store.uniSendStreams.value.map((stream) => (
            <UniSendStreamItem key={stream.id} streamId={stream.id} />
          ))}
        </div>
      )}
    </div>
